function NavigationController(inputWindowType) {
	this.windowStack = [];
	this.windowType = (inputWindowType) ? inputWindowType: require('/ui/handheld/ios/ApplicationWindow');
};
...
NavigationController.prototype.open = function(/*Ti.UI.Window*/windowToOpen) {
	...
	if (this.windowStack.length === 1) {
		if (Ti.Platform.osname === 'android') {
			...
			var containerWindow = this.createWindow(windowToOpen.title);
			containerWindow.add(this.navGroup);
			Ti.API.info('First window opened is ' + containerWindow.title + '\n');
...
NavigationController.prototype.back = function() {
	Ti.API.log("NavController Back function. customized for back button and other purposes.");
	if (this.windowStack.length > 1) {
		// setup chain reaction by setting up the flags on all the windows
		//for (var i = this.windowStack.length - 1; i > 1; i--)
		//{
        ...
        // start chain reaction, close first window
		(this.navGroup) ? this.navGroup.close(this.windowStack[this.windowStack.length - 1]) : this.windowStack[this.windowStack.length - 1].close();
		this.windowStack[this.windowStack.length -1] = null;
		this.windowStack.pop();
	}
	//Ti.API.log("Back. Stack: " + this.windowStack.map(function(v) {return v.title}));
};
